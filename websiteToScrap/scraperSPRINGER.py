...
def goToNextPage(driver, data, page):
    domain = data["domain"]
    query = data["query"]
    driver.google_get(f"{domain}/search?query={query}&openAccess=true&sortBy=relevance&page={page}") 
    time.sleep(10)
...
def getPageMax(driver: Driver):
    driver.click("button.cc-button.cc-button--secondary.cc-button--contrast.cc-banner__button.cc-banner__button-accept")
    pagination_element = driver.select("span[data-test='results-data-total']").text
    pagination_element = pagination_element.split(" ")
    pagination_element = pagination_element[3].replace(",", "")
...
def getArticles(driver, data):
    listTitles = []
    listAll = []
    articles = driver.select_all("a.app-card-open__link")
    for article in articles:
        title = (article.select("span").text).lower()
        isValid = True
        for listMotsVarValid in data["validRecherche"]:   
            ...
            if isAtLeastOneValid == False: 
                ...
                break
        if(isValid == True):
            hrefArt = (article.href).split('/')
            linkDownload = data["domain"] + '/content/pdf/' + hrefArt[2] + '/' + hrefArt[3] + '.pdf'
...
"""
def getPDF_SPRINGER(source_folder, download_folder, motRecherche, validRecherche):
    query = "%20".join(motRecherche)
    domain = f"https://link.springer.com"
    donnees = {
